...
6.  **`BONUS_*`**: These states render the self-contained bonus game components. When a bonus game finishes, it calls its `onComplete` callback to return the winnings to `game.tsx`.
