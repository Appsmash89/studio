...
export const TOP_SLOT_LEFT_REEL_ITEMS = ['1', '2', '5', '10', 'Coin Flip', 'Pachinko', 'Cash Hunt', 'Crazy Time'];
...
const Reel = ({ items, result, isSpinning }: { items: (string | number)[], result: string | number | null, isSpinning: boolean }) => {
    ...
    useEffect(() => {
        ...
        if (isSpinning) {
            ...
            reelRef.current.classList.remove('transition-transform', 'duration-[3s]', 'ease-out');
            ...
            if (result !== null) {
                ...
                reelRef.current.classList.add('transition-transform', 'duration-[3s]', 'ease-out');
